import os           # 用來處理電腦檔案系統
import datetime     # 用來處理時間
from duckduckgo_search import DDGS  # 網路搜索工具

# Google Calendar 專用零件，用來跟 Google 伺服器進行安全握手的
...
from googleapiclient.discovery import build       # 真正用來操作行事曆
...
class LoveTools:
    ...
    def load_knowledge_base(cls):
        ...
        """
        # 如果快取裡有資料，直接回傳，不需要再讀一次硬碟
        if cls._knowledge_cache: return cls._knowledge_cache

        knowledge_dir = "knowledge" # 資料放在這個資料夾
        data = {}
    ...
    def search_love_strategy(query):
        ...
        try:
            ...
            for category, content in kb.items():
                # 簡單的關鍵字比對：如果 query 是「天蠍座」，而資料本裡有這三個字
    ...
    def sync_to_google_calendar(title, start_time, duration="1h"):
        ...
        """
        # 定義權限：需要「讀取與寫入」行事曆的權限
        SCOPES = ['https://www.googleapis.com/auth/calendar']
        creds = None
        
        # 尋找鑰匙：
        # token.json 就像是長期識別證，有了之後就不用每次都重新登入
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                # 第一次使用，必須要有 credentials.json (授權書)
        ...
        try:
            ...
            # 處理時間，電腦只認識標準格式 (ISO Format)，AI 給的通常不標準，需要對時間進行標準化
            try:
            ...
            # Google 規定的固定填寫格式，不能亂改
            event = {
                'summary': title,
                'description': '💖 您的戀愛軍師為您精心安排的行程 💖',
                'start': {
                    'dateTime': start_dt.isoformat(),
                    'timeZone': 'Asia/Taipei', # 時區一定要設對，不然會以 GMT+0 爲準
                },
                'end': {
